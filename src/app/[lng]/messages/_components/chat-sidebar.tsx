...
const ChatListItem = ({ chat, isActive, lng }: { chat: ChatWithRecipient; isActive: boolean; lng: string; }) => {
    if (!chat.recipient) {
        return (
    ...
    return (
        <Link href={`/${lng}/messages/${chat.id}`} className={cn(
...
export function ChatSidebar({ lng }: { lng: string }) {
    ...
    if (authLoading || loading) {
        return (
            <div className="w-80 border-r p-4 space-y-4">
                {[...Array(5)].map((_, i) => <ChatListItem key={i} chat={{} as any} isActive={false} lng={lng} />)}
            </div>
        )
    }
    ...
    return (
        <aside className="w-80 border-r p-2">
            ...
            <div className="space-y-1">
                {chats.length > 0 ? (
                    chats.map(chat => (
                        <ChatListItem key={chat.id} chat={chat} isActive={chat.id === activeChatId} lng={lng}/>
                    ))
                ) : (
