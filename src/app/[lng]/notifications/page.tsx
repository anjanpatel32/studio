...
export default function NotificationsPage({ params }: { params: {lng: string}}) {
    const { lng } = params;
    const { user } = useAuth();
    ...
    return (
      <div className="container mx-auto max-w-2xl p-4 md:p-8">
        <h1 className="text-3xl font-bold mb-6 text-left">Notifications</h1>
                ...
                <Card key={notif.id} className="hover:bg-muted/50 transition-colors">
                    <CardContent className="p-4 flex items-center space-x-4">
                        <div className="relative shrink-0">
                            {notif.user ? (
                                <Link href={`/${lng}/profile/${notif.userId}`}>
                        ...
                        <div className="flex-1">
                            <p className="text-sm text-left">
                                {notif.user && <Link href={`/${lng}/profile/${notif.userId}`} className="font-semibold hover:underline">{notif.user.name}</Link>}{' '}
                                {notif.text}
                            </p>
                        ...
                        <div className="flex items-center gap-2 ml-auto shrink-0">
                            {notif.reelThumbnailUrl && (
                                <Link href={`/${lng}/reel/${notif.reelId}`}>
                                    <Image 
                            ...
                            {notif.type === 'follow' && (
                                <Button asChild size="sm" variant="secondary">
                                    <Link href={`/${lng}/profile/${notif.userId}`} className="whitespace-nowrap">
                                        View Profile
                                    </Link>
                                </Button>
                            )}
