...
import { updateUserSetting } from './actions';
import { useTranslation } from '@/app/i18n/client';


const SettingsItem = ({ icon, title, description, onClick, children }: { icon: React.ReactNode; title: string; description: string; onClick?: () => void; children?: React.ReactNode }) => (
  <div
    className="w-full flex items-center p-4 rounded-lg text-left"
  >
    ...
    {children ? children : (
        <Button variant="ghost" size="icon" onClick={onClick} className="hover:bg-muted/50 disabled:opacity-50 disabled:cursor-not-allowed">
...
const SettingsLinkItem = ({ icon, title, description, href }: { icon: React.ReactNode; title: string; description: string; href: string }) => (
    <Link href={href} passHref legacyBehavior>
        <a className="w-full flex items-center p-4 rounded-lg hover:bg-muted/50 transition-colors text-left">
            <div className="mr-4 text-primary">{icon}</div>
            <div className="flex-1">
            <p className="font-semibold">{title}</p>
            <p className="text-sm text-muted-foreground">{description}</p>
    ...
    <div className="container mx-auto max-w-3xl p-4 md:p-8">
      ...
      <Card className="mt-8">
        ...
        <CardContent className="divide-y">
            ...
            <SettingsItem icon={<CreditCard />} title={t('payments')} description="Manage payout methods and history" onClick={() => placeholderClick('Payment Settings')} />
            <SettingsItem 
                icon={<Palette />} 
                title={t('theme')} 
